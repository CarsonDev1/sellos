...
const FEATURED_IDS = ["ao-01", "quan-01", "giay-01", "pk-01"];
...
export default function ShopFeatured() {
  ...
  return (
    <section id="san-pham-ban-chay" className="py-20 px-4 bg-slate-50">
      <div className="container mx-auto max-w-7xl">
        ...
        >
          <div>
            <span className="text-rose-600 text-sm font-semibold uppercase tracking-wider">Bán chạy nhất</span>
          ...
          <Link
            href="#danh-muc"
            className="hidden sm:inline-flex text-sm text-rose-600 font-semibold hover:underline"
          >
            Xem tất cả →
          </Link>
        </motion.div>
          ...
          {featured.map((product, i) => (
            ...
            >
              <Link href={`/templates/shop-online/san-pham/${product.id}`} className="block relative overflow-hidden aspect-[3/4]">
                ...
                />
                {product.badge && (
                  <span className={`absolute top-2.5 left-2.5 text-white text-[10px] font-bold px-2 py-0.5 rounded-full ${product.badgeColor}`}>
                    {product.badge}
                  </span>
                )}
