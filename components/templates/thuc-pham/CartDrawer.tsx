            ...
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {items.length === 0 ? (
                ...
                items.map((item) => (
                  ...
                  >
                    <Link href={`/templates/thuc-pham/san-pham/${item.product.id}`} onClick={closeCart}>
                      <img
                        src={item.product.images[0]}
                        alt={item.product.name}
                        className="w-18 h-18 w-[72px] h-[72px] rounded-xl object-cover flex-shrink-0 border border-slate-100"
                      />
                    </Link>
                    <div className="flex-1 min-w-0">
                      <Link
                        ...
                        onClick={closeCart}
                        className="font-semibold text-slate-900 text-sm leading-snug hover:text-green-700 transition-colors line-clamp-2"
                      >
                        {item.product.name}
                      </Link>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="w-7 h-7 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:border-green-400 hover:text-green-700 transition-colors text-sm"
                          >
                            −
                          </button>
            ...
            {items.length > 0 && (
              <div className="px-5 py-4 border-t border-slate-100 space-y-3 bg-white">
                ...
                <div className="flex justify-between font-heading font-bold text-slate-900">
                  <span>Tổng cộng</span>
                  <span className="text-green-700 text-lg">{total.toLocaleString("vi-VN")}₫</span>
                </div>
                <div className="grid grid-cols-2 gap-2 pt-1">
                  <Link
                    href="/templates/thuc-pham/gio-hang"
                    onClick={closeCart}
                    className="py-3 rounded-xl border border-slate-200 text-slate-700 text-sm font-semibold text-center hover:border-green-400 hover:text-green-700 transition-colors"
                  >
                    Xem giỏ hàng
                  </Link>
                  <Link
                    ...
                    onClick={closeCart}
                    className="py-3 rounded-xl bg-green-600 hover:bg-green-700 text-white text-sm font-semibold text-center transition-colors shadow-sm"
                  >
                    Thanh toán
                  </Link>
                </div>
